...
// Generates a downscaled thumbnail blob for performance optimization
export function createThumbnail(file, maxDim = 400) {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          reject(new Error('Thumbnail canvas-to-blob failed'));
        }
      }, 'image/jpeg', 0.75);
    };
    img.onerror = (e) => {
...
// Generiert einen 8x8 Graustufen-Fingerabdruck eines Bildes zur Duplikaterkennung
export function getImageFingerprint(blob) {
  return new Promise((resolve) => {
    const img = new Image();
...
// Vergleicht zwei Fingerabdrücke und gibt die mittlere quadratische Abweichung (MSE) zurück
export function compareFingerprints(fp1, fp2) {
  if (!fp1 || !fp2 || fp1.length !== fp2.length) return 10000;
  let sumSqDiff = 0;
  for (let i = 0; i < fp1.length; i++) {
    const diff = fp1[i] - fp2[i];
    sumSqDiff += diff * diff;
  }
  return sumSqDiff / fp1.length;
}
