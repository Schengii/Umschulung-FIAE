...
export default function CloudDevOpsLab() {
  ...
  const activeModule = CLOUD_MODULES.find(m => m.id === selectedId) || CLOUD_MODULES[0];

  const handleCopy = () => {
    navigator.clipboard.writeText(activeModule.pipelineYaml);
    setCopied(true);
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto', paddingBottom: '60px' }}>
      <div className="glass-panel" style={{ padding: '32px', marginBottom: '24px', border: '2px solid var(--accent-teal)' }}>
        <h1 style={{ fontSize: '2.2rem', fontWeight: '800', marginBottom: '8px', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Cloud size={32} style={{ color: 'var(--accent-teal)' }} /> Cloud Infrastructure & DevOps Playground
        </h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '1.05rem' }}>
          Erstelle CI/CD Pipelines (GitHub Actions) & AWS Lambda Serverless Funktionen.
        </p>
      </div>
        ...
        {CLOUD_MODULES.map((m) => (
          <button
            ...
            style={{
              ...
              fontWeight: '700',
              fontSize: '0.95rem',
              background: selectedId === m.id ? 'var(--accent-teal)' : 'var(--bg-card)',
              color: selectedId === m.id ? '#ffffff' : 'var(--text-main)',
              border: selectedId === m.id ? '2px solid var(--accent-teal)' : '2px solid var(--border-color)',
              cursor: 'pointer',
              whiteSpace: 'nowrap'
            }}
          >
      ...
      <div className="glass-panel" style={{ padding: '32px' }}>
        ...
        <div className="code-window">
          <div className="code-header">
            <span>{activeModule.title} Pipeline Code</span>
          ...
          <pre className="code-body">
            <code>{activeModule.pipelineYaml}</code>
          </pre>
        </div>
      </div>
    </div>
